...
def test_bucket_for_matches_longest_prefix():
    ...
    assert checker.bucket_for("src.gia.gas.runtime") == "gas"
...
def test_bucket_for_also_matches_installed_package_names():
    ...
    assert checker.bucket_for("gas_protocol.contracts") == "gas"
    assert checker.bucket_for("gia_gas_adapter.mapping") == "gas"
...
@pytest.mark.parametrize(
    ...
    [
        # GIA-to-GAS: forbidden per the PR 13 exit criteria.
        (
...
def test_main_returns_nonzero_on_forbidden_import(tmp_path, capsys):
    ...
    (src_root / "forbidden.py").write_text("from src.gia.gas import GasRuntime\n")
