"""Domain logic — Havoc Engine mechanics for Eat the Reich (PR 18).

Moved here from ``src/gia/domain.py``'s ``HavocEngine`` — the concrete
game-mechanics half of that module. The domain-neutral error hierarchy
stays in ``gia_core.errors`` (PR 14); ``src/gia/domain.py`` re-exports both
this module's ``HavocEngine`` and the ``gia_core.errors`` names for
