"""Havoc-specific JSON-schema wiring (PR 4 of the GIA/GAS 2.0 plan; moved
here from ``src/gia/commands/schema.py`` in PR 18).

The generic JSON-schema machinery lives in ``gia_core.schema`` — it takes
an ``optional_parameters`` set explicitly rather than assuming one. This
module binds that parameter to ``OPTIONAL_PARAMETERS``, the kernel-wide set
of Havoc input-schema fields that are optional (currently only
``BuildDicePoolCommand``'s ``equipment_names``/``ability_name``/
``bonus_dice``), preserving the exact behavior every existing call site
(``havoc_domain.kernel``'s affordance/capability rendering) already relies
